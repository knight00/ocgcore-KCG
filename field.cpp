		...
		///kdiy///////////
			if (pcard->current.location == LOCATION_DECK) {
				...
				if(message) {
                    ...
                    message->write<bool>(true);
                    message->write<bool>(location == LOCATION_MZONE && pcard->is_affected_by_effect(EFFECT_SANCT_MZONE));
                    ///kdiy///////////
				} else {
					pcard->fieldid = infos.field_id++;
					if(check_unique_onfield(pcard, pcard->current.controler, pcard->current.location))
...
void field::swap_card(card* pcard1, card* pcard2, uint8_t new_sequence1, uint8_t new_sequence2) {
	...
	//////kdiy//////////
        ...
        message->write<bool>(false);
        message->write<bool>(true);
        message->write<bool>(loc1 == LOCATION_MZONE && pcard1->is_affected_by_effect(EFFECT_SANCT_MZONE));
        ///kdiy///////////
		message = pduel->new_message(MSG_MOVE);
		message->write<uint32_t>(pcard2->data.code);
		message->write(info2);
        ...
        message->write<bool>(false);
        message->write<bool>(true);
        message->write<bool>(loc2 == LOCATION_MZONE && pcard2->is_affected_by_effect(EFFECT_SANCT_MZONE));
        ///kdiy///////////
	} else {
		auto message = pduel->new_message(MSG_MOVE);
		message->write<uint32_t>(pcard2->data.code);
...
int32_t field::get_tofield_count(card* pcard, uint8_t playerid, uint8_t location, uint32_t uplayer, uint32_t reason, uint32_t zone, uint32_t* list) {
	...
	if(location == LOCATION_MZONE && is_player_affected_by_effect(playerid, EFFECT_ORICA)) {
		...
		if(!((zone >> 8) & 0xff))
		    ...
		    flag2 = ((flag2 | ~zone) >> 8) & 0x1f;
		count += 5 - field_used_count[flag2];
		flag = ((flag & 0xff) | ((flag2 & 0x1f) << 8)) & 0x1fff;
	}
	if(location == LOCATION_SZONE && is_player_affected_by_effect(playerid, EFFECT_SANCT)) {
		...
		count += 5 - field_used_count[flag2];
		flag = ((flag2 & 0x1f) | (flag & 0xff00)) & 0xff1f;
	}
	///////////kdiy////////
	if(location == LOCATION_MZONE)
		flag |= (1u << 5) | (1u << 6);
